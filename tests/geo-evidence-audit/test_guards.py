...
ALLOWED_STDLIB = {
    "argparse", "ast", "csv", "dataclasses", "fnmatch", "html", "json",
    "os", "pathlib", "re", "sys", "typing", "unicodedata", "urllib",
}
INTRA_SKILL = {"audit", "extract", "geo_data", "report", "rules"}
# Modules that would imply network access or process spawning -- never allowed.
FORBIDDEN = {
...
def test_scripts_have_no_network_or_spawn_calls():
    # import-level bans are enforced in the test above; here we catch raw calls
    # like os.system/os.popen. Pattern kept free of nouns that appear in
    # docstrings (e.g. "network requests." must not match).
    banned = re.compile(r"urllib\.request|urlopen|\bsocket\b|http\.client|subprocess|os\.system|os\.popen")
    for path in _script_files():
...
def test_openai_yaml_declares_name_and_exit_codes():
    ...
    assert "name: geo-evidence-audit" in text
